...
import { ProviderList } from '@boto/extension/providers/provider.list';
import { fetchCookie } from '@boto/extension/utils/load.cookie';
...
export const PopupContainerContainer: FC = () => {
  ...
  if (!url) {
    return (
      <div className="text-4xl">This website is not supported by Boto</div>
    );
  }
...
export const PopupContainer: FC<{ url: string }> = (props) => {
  ...
  if (!isLoggedIn) {
    return <div className="text-4xl">You are not logged in to Boto</div>;
  }
